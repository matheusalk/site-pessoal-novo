$input_file = "c:\Users\PC\Desktop\site ebook novo\index.html"
$output_file = "c:\Users\PC\Desktop\site ebook novo\design-system-completo.html"

$content = [System.IO.File]::ReadAllText($input_file)
$regex = [regex]::new("(?s)(<style>.*?</style>)")
$match = $regex.Match($content)

if ($match.Success) {
    $style_block = $match.Groups[1].Value
    $html_docs = @"
<!DOCTYPE html>
<html lang="pt-BR">
<head>
...
.ds-code { display: block; width: 100%; background: var(--color-bg-elevated); padding: 24px; border-radius: var(--r-md); font-family: monospace; font-size: var(--text-13); color: var(--color-text-secondary); overflow-x: auto; margin-top: 16px; border: 1px solid var(--color-border-subtle); white-space: pre; line-height: 1.6; }
...
<div class="ds-container">
  ...
  <section class="ds-section">
    ...
    <div class="ds-card-dark">
      ...
      <hr style="width: 100%; border: none; border-top: 1px solid var(--color-border-subtle); margin: 32px 0;">
  ...
  <section class="ds-section">
    ...
    <div class="ds-card-demo">
      ...
      <div class="ds-row" style="margin-top: 16px;">
        <div class="hero-price-block" style="margin-bottom: 0;">
    ...
    <div class="ds-card-dark">
      ...
      <div class="bonus-card" style="margin-top: 0; max-width: 600px;">
        ...
        <div>
          ...
          <div class="bonus-text">Corpo de texto que explica qual é a oferta ou bônus com destaque visual.</div>
        </div>
      </div>
  ...
  <section class="ds-section">
    <h2 class="ds-title">7. Painel de Acordeão (FAQ)</h2>
    <div class="ds-card-demo">
  ...
  <section style="text-align: center; margin-top: 120px;">
    <div style="font-family: var(--font-display); font-size: var(--text-24); font-weight: var(--w-bold); color: var(--color-text-primary); margin-bottom: 24px;">Pronto para usar.</div>
    <p style="color: var(--color-text-muted); max-width: 500px; margin: 0 auto;">Todo o CSS fundamental está acoplado no &lt;head&gt; deste arquivo, basta você copiar a tag &lt;style&gt; (até a linha 1700) e usar livremente nos seus próximos projetos.</p>
  </section>
...
"@
    [System.IO.File]::WriteAllText($output_file, $html_docs, [System.Text.Encoding]::UTF8)
    Write-Host "Design System gerado em $($output_file)"
} else {
    Write-Error "Não encontrou a tag <style>."
}
